...
_chprSRPF:     file format elf32-i386
...
main(int argc, char *argv[])
{
  ...
  2b:	50                   	push   %eax
  2c:	56                   	push   %esi
  2d:	e8 30 03 00 00       	call   362 <chprSRPF>
    exit();
...
putc(int fd, char c)
{
 ...
 3f8:	e8 a5 fe ff ff       	call   2a2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 468:	50                   	push   %eax
 469:	57                   	push   %edi
 46a:	e8 33 fe ff ff       	call   2a2 <write>
...
putc(int fd, char c)
{
 ...
 5af:	50                   	push   %eax
 5b0:	57                   	push   %edi
 5b1:	e8 ec fc ff ff       	call   2a2 <write>
...
free(void *ap)
{
 ...
 617:	89 08                	mov    %ecx,(%eax)
  freep = p;
 619:	a3 e8 09 00 00       	mov    %eax,0x9e8
}
 61e:	5b                   	pop    %ebx
 61f:	5e                   	pop    %esi
 620:	5f                   	pop    %edi
 ...
 63a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 63d:	a3 e8 09 00 00       	mov    %eax,0x9e8
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 659:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 65c:	8b 15 e8 09 00 00    	mov    0x9e8,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
 ...
 6d6:	e8 e5 fe ff ff       	call   5c0 <free>
  return freep;
 6db:	8b 15 e8 09 00 00    	mov    0x9e8,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 727:	09 00 00 
    base.s.size = 0;
 72a:	b8 ec 09 00 00       	mov    $0x9ec,%eax
 72f:	c7 05 f0 09 00 00 00 	movl   $0x0,0x9f0
 736:	00 00 00 
